...
--      weizPVP_CoreBar.BG:SetVertexColor(unpack(NS.Options.Frames.Header.BackgroundColor))
	weizPVP_CoreBar.BG:SetVertexColor(0,0,0,0)
    ...
    --|> CORE FRAME
      weizPVP_CoreFrame:SetResizeBounds(140, (NS.Options.Bars.RowHeight + NS.Options.Frames.Header.Height), 600, (((NS.Options.Bars.RowHeight + NS.Options.Bars.VerticalSpacing) * NS.Options.Bars.MaxNumBars) + NS.Options.Frames.Header.Height - 1))
...
--      weizPVP_CoreFrame.ScrollFrame.BG:SetAlpha(NS.Options.Frames.BackgroundColor.a)
	local fadeAlphaF = (NS.Options.Frames.BackgroundColor.a)
	if fadeAlphaF == nil then
	fadeAlphaF = 0
	elseif fadeAlphaF < 0 then
	fadeAlphaF = 0
	elseif fadeAlphaF > 1 then
	fadeAlphaF = 1
	end

	weizPVP_CoreFrame.ScrollFrame.BG:SetAlpha(fadeAlphaF)
...
--        weizPVP_CoreBar.BG:SetAlpha(NS.Options.Frames.Header.BackgroundColor.a)
	local fadeAlphaH = (NS.Options.Frames.Header.BackgroundColor.a)
	if fadeAlphaH == nil then
	fadeAlphaH = 0
	elseif fadeAlphaH < 0 then
	fadeAlphaH = 0
	elseif fadeAlphaH > 1 then
	fadeAlphaH = 1
	end

	weizPVP_CoreBar.BG:SetAlpha(fadeAlphaH)
    end
    initializing = false
end
